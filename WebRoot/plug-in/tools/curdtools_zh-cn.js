...
function srcAdd(title,url,id,gname,width,height,isRestful) {
	gridname=id;
...
function querySrc(title,addurl,gname,width,height) {
	var name=gname;
	if(width=="100%" || height=="100%"){
		width = window.top.document.body.offsetWidth;
		height =window.top.document.body.offsetHeight-100;
	}
	openwindow(title,addurl,name,width,height);
...
function update(title,url, id,width,height,isRestful) {
	...
	if (rowsData.length>1) {
	 ...
	 if(rowsData[0].billstatus == 1 ||rowsData[0].billstatus == 2|| rowsData[0].billstatus == -1 ) {
		tip("已审核的单据不能编辑！");
		return;
	}
	if(isRestful!='undefined'&&isRestful){
		...
		url += '&load=update&id='+rowsData[0].id;
	}
	createwindow(title,url,width,height);
}
 ...
 * @return
 */
function approveALLSelect(title,url,gname) {
	gridname=gname;
    var ids = [];
    var rows = $("#"+gname).datagrid('getSelections');
    if (rows.length > 0) {
    	$.dialog.setting.zIndex = getzIndex(true);
    	$.dialog.confirm('你确定审核下单已选数据吗?', function(r) {
		   if (r) {
				for ( var i = 0; i < rows.length; i++) {
					ids.push(rows[i].id);
				}
