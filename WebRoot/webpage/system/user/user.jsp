...
//            var orgIds = $("#orgSelect").combobox("getValues");
        ...
        $(function() {
            ...
            //       $("#orgSelect").combobox("setValues", ${orgIdList});
             var orgIds = $("#orgSelect").combotree("getValues");
            $("#orgSelect").combotree("setValues", orgIds);
        }); 
//       update-start--Author:zhangguoming  Date:20140826 for：将combobox修改为combotree

		function openDepartmentSelect() {
			$.dialog.setting.zIndex = 9999; 
			$.dialog({content: 'url:departController.do?departSelect', zIndex: 2100, title: '客商', lock: true, width: '400px', height: '350px', opacity: 0.4, button: [
		...
		$(function(){
			...
			//$("#orgSelect").combotree("setValues", document.getElementById("orgIds").value);
			$("#orgSelect").combotree("setValue", document.getElementById("orgIds").value);
		});
	...
	<table style="width: 600px;" cellpadding="0" cellspacing="1" class="formtable">
		...
		<tr>
			...
			<td class="value" width="10%">
                <input id="realName" class="inputxt" name="realName" value="${user.realName }" datatype="s2-50">
		...
		<tr>
			<td align="right"><label class="Validform_label"> 客商档案: </label></td>
			<td class="value">
				 <%--update-start--Author:jg_renjie  Date:20160320 for：#942 【组件封装】组织机构弹出模式，目前是列表，得改造成树方式--%>
                <%--update-start--Author:zhangguoming  Date:20140826 for：将combobox修改为combotree--%>
