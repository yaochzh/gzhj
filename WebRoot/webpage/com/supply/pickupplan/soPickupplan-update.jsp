...
<script type="text/javascript">
  ...
  function myclick(id)	{
	  var carrierid = document.getElementById("carrier").value;
  		$.ajax({
      		type : "POST",
      		url : "bdCarrierController.do?queryCarrierCodeById&carrierid="+carrierid+"",// 请求的action路径
      		...
      		success : function(data) {
      			...
      			if (d.success) {
    				var carriercode = d.attributes.carriercode;
  ...
  function checkvehicle(){ 
	  var soordercode = document.getElementById("soordercode").value;
	   var planassnum = document.getElementById("planassnum").value;
	   var soordernum = document.getElementById("soordernum").value;
	   var soordernum = document.getElementById("soordernum").value;
	   var id = document.getElementById("id").value;
	   var isFlag = "1";
	   var surplusnumNew = 0;
	   if(!(planassnum > 0)) {
			$.messager.alert('错误', "计划量必须为正数！");  
			return false;
	   }
	  	$.ajax({
	  		type : "POST",
	  		async:false, 
	  		url : "soPickupplanController.do?checkPlanassnum&planassnum="+planassnum+"&soordercode="+soordercode+"&soordernum="+soordernum+"&id="+id+"",// 请求的action路径
	  		error : function() {// 请求失败处理函数
	  			$.messager.alert('错误', "计划量校验错误！");  
	  		return false;
	  		},
	  		success : function(data) {
	  			var d = $.parseJSON(data);
	  			if (d.success) {
	  				var isOK= d.attributes.isOK;
	  				if(!(isOK==1)) {
	  					isFlag = "0";
	  					surplusnumNew = d.attributes.surplusnumNew;
	  				}
	  			}		
	  		}
	  	});
	  	if(isFlag == 1) {
	  		$.messager.alert('错误', "计划量超订单量或剩余数量为负数！");  
	  		return false;
	  	}
	  document.getElementById("surplusnum").value = surplusnumNew;
		...
		<table cellpadding="0" cellspacing="1" class="formtable">
			<tr>
				...
				<td class="value"><input id="pickupplancode"
					name="pickupplancode" type="text" style="width: 150px"
				...
				<td class="value"><input id="soordercode" name="soordercode"
					...
					value='${soPickupplanPage.soordercode}'> <span
			...
			<tr>
				...
				<td class="value"><input id="materialcode" name="materialcode"
					type="text" style="width: 150px" class="inputxt"
					value='${soPickupplanPage.materialcode}'> <span
					class="Validform_checktip"></span> <label class="Validform_label"
				...
				<td class="value"><input id="materialname" name="materialname"
					type="text" style="width: 150px" class="inputxt"
					value='${soPickupplanPage.materialname}'> <span
					class="Validform_checktip"></span> <label class="Validform_label"
				...
				<td class="value"><input id="spec" name="spec" type="text"
					...
					value='${soPickupplanPage.spec}'> <span
					class="Validform_checktip"></span> <label class="Validform_label"
			...
			<tr>
				...
				<td class="value"><input id="unit" name="unit" type="text"
					style="width: 150px" class="inputxt"
					value='${soPickupplanPage.unit}'> <span
					class="Validform_checktip"></span> <label class="Validform_label"
				...
				<td class="value"><input id="customername" name="customername"
					type="text" style="width: 150px" class="inputxt"
					value='${soPickupplanPage.customername}'> <span
					class="Validform_checktip"></span> <label class="Validform_label"
				...
				<td class="value"><t:dictSelect id="delivercustomer"
						...
						dictField="id" dictText="departname"
			...
			<tr>
				...
				<td class="value"><input id="store" name="store" type="text"
					style="width: 150px" class="inputxt"
					value='${soPickupplanPage.store}'> <span
					class="Validform_checktip"></span> <label class="Validform_label"
				...
				<td class="value"><input id="surplusnum" name="surplusnum"
					...
					value='${soPickupplanPage.surplusnum}'> <span
					class="Validform_checktip"></span> <label class="Validform_label"
				...
				<td class="value"><input id="expiredate" name="expiredate"
					type="text" style="width: 150px" class="Wdate"
					onClick="WdatePicker()"
					value='<fmt:formatDate value='${soPickupplanPage.expiredate}' type="date" pattern="yyyy-MM-dd"/>' datatype="*">
					<span class="Validform_checktip"></span> <label
			...
			<tr>
				...
				<td class="value"><input id="planassnum" name="planassnum"
					type="text" style="width: 150px" class="inputxt"
					value='${soPickupplanPage.planassnum}' onchange="checkPlanassnum()" > <span
					class="Validform_checktip"></span> <label class="Validform_label"
			...
			<td class="value">
		      <t:dictSelect id="carrier" field="carrier" type="list" dictCondition="fuck" dictTable="bd_carrier" dictField="id" dictText="carriername" defaultVal="${soPickupplanPage.carrier}" hasLabel="false" title="承运单位" ></t:dictSelect>    
		...
		<tbody id="add_soPickupplanB_table_template">
			<tr>
				...
				<td align="left"> <input id="soPickupplanBList[#index#].vehiclelicense"
					...
					type="text" class="inputxt" style="width:120px;"  readOnly="true" datatype="*"></input><label class="Validform_label" style="display: none;">车牌号</label>
				</td>
				<td align="left"><input
