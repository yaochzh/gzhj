...
public class SoPickupplanController extends BaseController {
	...
	HqlGenerateUtil.installHql(cq, soPickupplan);
	try {
	...
	soPickupplan = (SoPickupplanEntity) this.systemService.getEntity(
	...
	try {
		...
		SoPickupplanEntity soPickupplan = (SoPickupplanEntity) this.systemService
			.getEntity(SoPickupplanEntity.class, id);
		this.soPickupplanService.delMain(soPickupplan);
		this.systemService.addLog(message, Globals.Log_Type_DEL,
			Globals.Log_Leavel_INFO);
	    }
	...
	List soPickupplanBList = soPickupplanPage.getSoPickupplanBList();
	AjaxJson j = new AjaxJson();
	String message = "添加成功";
	try {
	...
	try {
	    this.soPickupplanService
		    .updateMain(soPickupplan, soPickupplanBList);
	...
	if (StringUtil.isNotEmpty(soPickupplan.getId())) {
		...
		soPickupplan.setMaterialcode(((Map) list.get(0))
			.get("cInvCode") == null ? "" : ((Map) list.get(0))
			.get("cInvCode").toString());
		soPickupplan.setMaterialname(((Map) list.get(0))
			.get("CINVNAME") == null ? "" : ((Map) list.get(0))
			.get("CINVNAME").toString());
		soPickupplan
			.setSpec(((Map) list.get(0)).get("CINVSTD") == null ? ""
				: ((Map) list.get(0)).get("CINVSTD").toString());
		soPickupplan.setCustomercode(((Map) list.get(0))
			.get("cCusCode") == null ? "" : ((Map) list.get(0))
			.get("cCusCode").toString());
		soPickupplan.setCustomername(((Map) list.get(0))
			.get("cCusName") == null ? "" : ((Map) list.get(0))
			.get("cCusName").toString());
		soPickupplan
			.setSoordernum(((Map) list.get(0)).get("IQUANTITY") == null ? BigDecimal.ZERO
				: BigDecimal.valueOf(Double
					.parseDouble(((Map) list.get(0)).get(
						"IQUANTITY").toString())));
		if ((soPickupplanEntitys != null)
			&& (soPickupplanEntitys.size() > 0)) {
		    BigDecimal orderNum = ((SoPickupplanEntity) soPickupplanEntitys
			    .get(0)).getSoordernum();
		    BigDecimal planassNums = BigDecimal.ZERO;
		    for (SoPickupplanEntity entity : soPickupplanEntitys) {
			planassNums = planassNums
				.add(entity.getPlanassnum() == null ? BigDecimal.ZERO
					: entity.getPlanassnum());
		    }
		    soPickupplan.setSurplusnum(orderNum.subtract(planassNums));
		    soPickupplan.setPlanassnum(orderNum.subtract(planassNums));
		} else {
		    soPickupplan.setSurplusnum(((Map) list.get(0))
			    .get("IQUANTITY") == null ? BigDecimal.ZERO
			    : BigDecimal.valueOf(Double.parseDouble(((Map) list
				    .get(0)).get("IQUANTITY").toString())));
		    soPickupplan.setPlanassnum(((Map) list.get(0))
			    .get("IQUANTITY") == null ? BigDecimal.ZERO
			    : BigDecimal.valueOf(Double.parseDouble(((Map) list
				    .get(0)).get("IQUANTITY").toString())));
		}
	...
	if (StringUtil.isNotEmpty(soPickupplan.getId())) {
	    soPickupplan = (SoPickupplanEntity) this.soPickupplanService
		    .getEntity(SoPickupplanEntity.class, soPickupplan.getId());
	    req.setAttribute("soPickupplanPage", soPickupplan);
	}
	if ((roleCodes.contains("admin")) || (roleCodes.contains("supper"))) {
	    return new ModelAndView("com/supply/pickupplan/soPickupplan-update");
	}
	return new ModelAndView("com/supply/pickupplan/soPickupplan-update-new");
    }
	...
	try {
	    List soPickupplanBEntityList = this.systemService.findHql(hql0,
		    new Object[] { id0 });
	...
	List listSoPickupplans = this.soPickupplanService
	...
	SoPickupplanEntity task = (SoPickupplanEntity) this.soPickupplanService
		.get(SoPickupplanEntity.class, id);
	if (task == null) {
	...
	Set failures = this.validator.validate(soPickupplanPage, new Class[0]);
	if (!failures.isEmpty()) {
	...
	SoPickupplanEntity soPickupplan = (SoPickupplanEntity) this.soPickupplanService
		.get(SoPickupplanEntity.class, id);
	this.soPickupplanService.delMain(soPickupplan);
    }
	...
	if ((StringUtil.isEmpty(createBy))
		&& (StringUtil.isEmpty(createByName))) {
	    return;
	}
	List<Map<String, Object>> list = this.soPickupplanU8Service
		.queryU8SoOrder(createBy.toString(), createByName.toString());
	CriteriaQuery cq = new CriteriaQuery(SoPickupplanEntity.class, dataGrid);
	...
	for (Map<String, Object> map : list) {
	    ...
	    entity.setSurplusnum(map.get("SURPLUSQUANTITY") == null ? BigDecimal.ZERO
		    : BigDecimal.valueOf(Double.parseDouble(map.get(
			    "SURPLUSQUANTITY").toString())));
	...
	HqlGenerateUtil.installHql(cq, poDeliveryplan);
    ...
    public ModelAndView querySrc(SoPickupplanBEntity soPickupplan,
	    HttpServletRequest req) {
	if (StringUtil.isNotEmpty(soPickupplan.getId())) {
	    soPickupplan = (SoPickupplanBEntity) this.soPickupplanService
		    .getEntity(PoDeliveryplanEntity.class, soPickupplan.getId());
	    req.setAttribute("soPickupplanPage", soPickupplan);
	}
	...
	List ls = this.commonService.getListByCriteriaQuery(cq,
	...
	if (!StringUtil.isEmpty(code)) {
	    VehicleEntity vehicle = (VehicleEntity) this.systemService
		    .getEntity(VehicleEntity.class, code);
	    if (vehicle != null) {
    ...
    private void updateCar(List<SoPickupplanBEntity> poDeliveryplanBList,
	    String status) {
	...
	sql = sql
		+ JeecgSqlUtil.buildSqlForIn("id",
			(String[]) carid.toArray(new String[0]));
	...
	String hql = "select ID, VEHICLELICENSE\n  from bd_vehicle\n where bd_vehicle.vehiclelicense not in\n       (select bd_vehicle.vehiclelicense\n          from so_pickupplan\n          left join so_pickupplan_b\n            on so_pickupplan.id = so_pickupplan_b.fk_id\n          inner join bd_vehicle\n            on so_pickupplan_b.vehiclelicense = bd_vehicle.id\n         where so_pickupplan.expiredate >= trunc(sysdate)\n           )\n   and bd_vehicle.vehiclelicense not in\n       (select bd_vehicle.vehiclelicense\n          from po_deliveryplan\n          left join po_deliveryplan_b\n            on po_deliveryplan.id = po_deliveryplan_b.fk_id\n          inner join bd_vehicle\n            on po_deliveryplan_b.vehiclelicense = bd_vehicle.id\n         where po_deliveryplan.expiredate >= trunc(sysdate)\n           ) and bsealflag <> 'Y' and ccarrier = '"
		+ code + "'";
	if (!StringUtil.isEmpty(code)) {
	    List vehicles = this.systemService.findObjForJdbc(hql, 1, 10000,
		    VehicleEntity.class);
	    if (vehicles != null) {
		for (int i = 0; i < vehicles.size(); i++) {
		    jsonObject
			    .put(((VehicleEntity) vehicles.get(i)).getId() == null ? ""
				    : ((VehicleEntity) vehicles.get(i)).getId()
					    .toString(),
				    ((VehicleEntity) vehicles.get(i))
					    .getVehiclelicense() == null ? ""
					    : ((VehicleEntity) vehicles.get(i))
						    .getVehiclelicense()
						    .toString());
		}
	...
	String soordernum = request.getParameter("soordernum") == null ? "0"
		: request.getParameter("soordernum").toString();
	...
	if (StringUtil.isEmpty(planassnum)) {
		...
		BigDecimal planassnums = BigDecimal.ZERO;
		BigDecimal ordernum = BigDecimal.valueOf(Double
			.parseDouble(soordernum));
		for (SoPickupplanEntity entity : soPickupplanEntitys) {
		    if (entity.getId().equals(id)) {
			surplusnumOld = entity.getSurplusnum() == null ? BigDecimal.ZERO
				: entity.getSurplusnum();
			planassnumOld = entity.getPlanassnum() == null ? BigDecimal.ZERO
				: entity.getPlanassnum();
			surplusnumNew = surplusnumOld.add(BigDecimal.valueOf(
				Double.parseDouble(planassnum)).subtract(
				planassnumOld));
		    } else {
			planassnums = planassnums
				.add(entity.getPlanassnum() == null ? BigDecimal.ZERO
		...
		if ((BigDecimal.valueOf(Double.parseDouble(planassnum))
			.compareTo(ordernum.subtract(planassnums)) > 0)
			|| (surplusnumNew.compareTo(BigDecimal.ZERO) < 0)) {
		    jsonObject.put("isOK", "1");
		}
		jsonObject.put("surplusnumNew", surplusnumNew);
	    } else if (BigDecimal.valueOf(Double.parseDouble(planassnum))
		    .compareTo(
			    BigDecimal.valueOf(Double.parseDouble(soordernum))) > 0) {
		jsonObject.put("isOK", "1");
	    }
	}
